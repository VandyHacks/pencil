...
EventController.addAttendee = function (EVENT_ID, attendee, callback) {
  UserController.getById(attendee, (err, user) => {
    ...
    Event.findOne({
      ...
      // allow multiple attendance for meals (track seconds) and check-in (assign new wristbands) only
      const disableMultipleAttendance = event.eventType !== 'Meal' && event.eventType !== 'CheckIn';
      if (disableMultipleAttendance && // prevent multiple attendance
